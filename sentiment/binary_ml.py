# Binary Machine Learning Sentiment Analysis Training Script (Positive/Negative only)
import os
import sys
import pandas as pd
import numpy as np
...
class Config:
    DATA_DIR = '../dataset'
    OUTPUT_DIR = 'outputs'
    CONFUSION_MATRIX_DIR = os.path.join(OUTPUT_DIR, 'binary_ml_confusion_matrix')
...
def create_confusion_matrix(y_true, y_pred, model_name, embedding_name, output_dir, accuracy=None):
    ...
    try:
        ...
        safe_embedding_name = embedding_name.replace("-", "_").replace("/", "_")
...
def create_performance_visualization(results_df, output_dir):
    ...
    fig.suptitle('ML Model Performance Comparison (Binary)', 
...
def create_results_table(results_df, output_dir):
    ...
    plt.title('ML Model Performance Results Table (Binary)', 
...
def main():
    print("=" * 80)
    print("🎯 BINARY ML ESTIMATION (Positive vs Negative)")
    print("=" * 80)
    ...
    # Generate ground truth with RoBERTa
    ...
    if roberta_tokenizer and roberta_model:
        ...
        if invalid_mask.sum() > 0:
            print(f"   ⚠️  {invalid_mask.sum()} invalid RoBERTa predictions...")
            df.loc[invalid_mask, "ground_truth_mc"] = df.loc[invalid_mask, "cleaned_text"].apply(detect_sentiment_advanced)
    ...
    # Use the full 3-class ground truth
    ...
    print(f"   📉 Filtered {original_len} -> {len(binary_df)} samples")
    ...
    if all_results:
        ...
        print(f"✅ Comparison charts and tables saved to {Config.CONFUSION_MATRIX_DIR}")
